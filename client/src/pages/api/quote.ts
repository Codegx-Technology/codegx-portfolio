...
// Define validation schema for the complex quote form
const complexQuoteSchema = z.object({
  // Business Type
...
// Define validation schema for the simple quote form
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (body.projectGoals) {
      // Complex quote form
      data = complexQuoteSchema.parse(body);
      ...
      // Compose email HTML for complex form
      emailHtml = `
      ...
      // Simple quote form
      data = simpleQuoteSchema.parse(body);

      // Compose email HTML for simple form
      emailHtml = `
        <h1>New Simple Quote Request from ${data.name}</h1>
        <p><strong>Submitted on:</strong> ${new Date().toLocaleString()}</p>
        ...
        <p><strong>Timeline:</strong> ${data.urgency}</p>
    ...
    const { data: emailData, error } = await resend.emails.send({
      ...
      subject: `New Quote Request: ${data.name} - ${body.projectGoals ? (data.recommendedService || 'AI Services') : 'Simple Quote'}`,
      html: emailHtml,
      reply_to: data.email,
    });
